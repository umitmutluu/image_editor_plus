...
import 'package:image_editor_plus/signature/signature_control.dart';
import 'package:image_editor_plus/signature/signature_painter.dart';
import 'package:image_editor_plus/signature/signature_view.dart';
...
class ImageCropper extends StatefulWidget {
  ...
  final List<o.ASAspectRatio> availableRatios;

  const ImageCropper({
    super.key,
    required this.image,
    this.availableRatios = const [
      o.ASAspectRatio(title: 'Freeform'),
      o.ASAspectRatio(title: '1:1', ratio: 1),
      o.ASAspectRatio(title: '4:3', ratio: 4 / 3),
      o.ASAspectRatio(title: '5:4', ratio: 5 / 4),
      o.ASAspectRatio(title: '7:5', ratio: 7 / 5),
      o.ASAspectRatio(title: '16:9', ratio: 16 / 9),
    ],
  });
        ...
        appBar: AppBar(
          actions: [
            IconButton(
              ...
              onPressed: () async {
                loadingScreen.show();
                final val = GeneratedImage(
                    screenshotController: screenshotController,
                    widget: widget,
                    selectedFilter: selectedFilter,
                    filterOpacity: filterOpacity);
                var data = await screenshotController.captureFromWidget(val);
...
class _ImageEditorDrawingState extends State<ImageEditorDrawing> {
  ...
  var fakeImageScreenController = ScreenshotController();
  final RenderRepaintBoundary repaintBoundary = RenderRepaintBoundary();
  final strokeWidthNotifier = ValueNotifier(2.0);
  ...
  void getImageSizes() {
    img.Image? image = img.decodeImage(Uint8List.fromList(widget.image.bytes));
    imageWidth = image?.width ?? MediaQuery.of(context).size.width.toInt();
    imageHeight = image?.height ?? MediaQuery.of(context).size.height.toInt();
  }
  ...
  Uint8List resizeImage(Uint8List data,int newWidth, int newHeight) {
    Uint8List resizedData = data;
    img.Image? imgs = img.decodeImage(data);
    img.Image resized = img.copyResize(imgs!, width: newWidth, height: newHeight);
            ...
            actions: [
              ...
              IconButton(
                ...
                onPressed: () async {
                  ...
                  if (widget.options.isFakeImage) {
                    loadingScreen.show();
                    var image =
                        await fakeImageScreenController.captureFromWidget(
                      buildFakeImage(),
                    );
                  final convertedImage = resizeImage(image, imageWidth  , imageHeight);

                  loadingScreen.hide();

                    if (!mounted) return;

                    return Navigator.pop(context, convertedImage);
                  }
                  loadingScreen.show();
  ...
  Widget buildFakeImage() {
    return Screenshot(
      controller: fakeImageScreenController,
      child: AspectRatio(
        aspectRatio: (imageWidth / imageHeight),
        child: Container(
